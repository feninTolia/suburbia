...
export const Controls = ({ wheels, decks, metals, className }: Props) => {
  ...
  }, [selectedWheel, selectedBolt, selectedDeck, selectedTruck, router]);
...
const Options = memo(({ title, selectedName, children }: OptionsProps) => {
  ...
  ({
    ...
    return (
      <li>
        <button
          className={clsx(
            'size-10 cursor-pointer rounded-full bg-black p-0.5 outline-white outline-2',
            selected && 'outline'
          )}
          onClick={onClick}
        >
